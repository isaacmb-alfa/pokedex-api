...
- [ ] 1. Introducir el estado centralizado y utilidades base en `script.js`
  - [ ] 1.1 Declarar el objeto `appState` con las propiedades `lastUrl`, `activeFilter`, `isTypeFilter`, `expectedCount` y `retryCallback`
    - Sustituye las variables sueltas `nextUrl` e `isFetching` existentes
    - _Requisitos: 1.5, 2.1, 4.1, 5.1_
  - [ ] 1.2 Implementar la función utilitaria `debounce(fn, delay)` en `script.js`
...
- [ ] 2. Implementar Skeleton Cards con efecto Shimmer (Requisito 1)
  - [ ] 2.1 Añadir las clases CSS `skeleton-card`, `.skeleton`, `.skeleton-img`, `.skeleton-text`, `.skeleton-text-sm` y la animación `@keyframes shimmer` con la clase `.shimmer` en `style.css`
    - _Requisitos: 1.1, 1.2_
  - [ ] 2.2 Crear las funciones `mostrarSkeletons(count)` y `eliminarSkeletons()` en `script.js`
  ...
  - [ ] 2.3 Integrar `mostrarSkeletons` y `eliminarSkeletons` en `fetchPokemonData`: llamar a `mostrarSkeletons` al inicio y a `eliminarSkeletons` antes de renderizar o mostrar el error
...
- [ ] 3. Implementar filtro activo en la navegación (Requisitos 2 y 9)
  - [ ] 3.1 Añadir las reglas CSS `.btn-header.active` y `#ver-todos.active` en `style.css`
  ...
  - [ ] 3.2 Crear la función `setFiltroActivo(id)` en `script.js` y conectarla al listener de `botonesHeader`
...
- [ ] 4. Checkpoint — Verificar skeleton, filtro activo y estado centralizado
  - Asegurarse de que los tests pasan y el estado `appState` se actualiza correctamente en cada interacción. Preguntar al usuario si surge alguna duda.

- [ ] 5. Implementar Error Card con botón "Reintentar" (Requisito 3)
  - [ ] 5.1 Crear la función `mostrarErrorCard(mensaje, retryFn)` en `script.js`
  ...
  - [ ] 5.2 Sustituir todos los bloques `catch` de `fetchPokemonData` y `searchPokemon` para que invoquen `mostrarErrorCard` en lugar de `alert()` o `console.error()`
    - Incluir el caso de búsqueda sin resultados (Requisito 3.4)
...
- [ ] 6. Implementar visibilidad del botón "Cargar Más" según el filtro (Requisito 4)
  - [ ] 6.1 Crear las funciones `ocultarCargarMas()` y `mostrarCargarMas()` en `script.js`
  ...
  - [ ] 6.2 Conectar `ocultarCargarMas` y `mostrarCargarMas` al listener de `botonesHeader`
...
- [ ] 7. Conectar el debounce al formulario de búsqueda (Requisito 5)
  - [ ] 7.1 Crear `debouncedSearch` aplicando la función `debounce` (de la tarea 1.2) sobre `searchPokemon` con 300 ms
  - [ ] 7.2 Actualizar el listener del formulario `#search-form` para invocar `debouncedSearch` en lugar de `searchPokemon` directamente
    - Ignorar el evento si el campo `#search-input` está vacío
    - _Requisitos: 5.1, 5.2, 5.3_

- [ ] 8. Implementar color de fondo de Cards por tipo y animación fadeIn (Requisitos 6 y 7)
  - [ ] 8.1 Añadir las 18 clases `{tipo}-bg` con `background-color` y `color` correspondientes en `style.css`, reutilizando las variables `--type-*` ya declaradas
    - _Requisitos: 6.1, 6.2, 6.3_
  - [ ] 8.2 Añadir el keyframe `@keyframes fadeIn` y la clase `.card-fade-in` en `style.css`
    - Inicio: `opacity: 0; transform: translateY(12px)` — fin: `opacity: 1; transform: translateY(0)`; duración 400 ms, `ease-out`
    - _Requisitos: 7.1, 7.2, 7.3_
  - [ ] 8.3 Modificar `imprimirPokemones` en `script.js` para asignar `{tipoPrincipal}-bg` y `card-fade-in` a `card.className`
...
- [ ] 9. Implementar hover pronunciado en las Cards (Requisito 8)
  - [ ] 9.1 Añadir al selector `.pokemon-card` en `style.css` las reglas `transition: transform 0.2s ease, box-shadow 0.2s ease` y `cursor: pointer`
    - _Requisitos: 8.2, 8.3_
  - [ ] 9.2 Añadir el selector `.pokemon-card:hover` en `style.css` con `transform: scale(1.05)` y `box-shadow: 0 8px 24px rgba(0,0,0,0.18)`
    - _Requisitos: 8.1_

- [ ] 10. Checkpoint — Verificar Cards, animaciones y hover
  - Asegurarse de que las Cards muestran el color correcto, el fadeIn funciona y el hover es perceptible. Preguntar al usuario si surge alguna duda.

- [ ] 11. Rediseñar el Modal estilo Pokédex (Requisito 10)
  - [ ] 11.1 Actualizar la estructura HTML del Modal en `index.html`: añadir `#modal-header` con las imágenes (`#modal-img-oficial`, `#modal-img-shiny`), título, tipos y contenedor de stats/habilidades/movimientos en `#modal-body`
    - _Requisitos: 10.1, 10.2_
  - [ ] 11.2 Añadir las reglas CSS para las Barras de Stat en `style.css`: selector genérico `.stat-bar` más los colores diferenciados por nombre de stat (`hp`, `attack`, `defense`, `special-attack`, `special-defense`, `speed`)
    - _Requisitos: 10.3_
  - [ ] 11.3 Refactorizar la función `showModal` en `script.js`:
...
- [ ] 12. Implementar animación de entrada y salida del Modal (Requisito 11)
  - [ ] 12.1 Añadir los keyframes `@keyframes modalEntrada` y `@keyframes modalSalida` en `style.css`, junto con las clases `.modal-content.modal-entrada` y `.modal-content.modal-salida`
    - `transform-origin: center top`
    - Entrada: 300 ms `ease-out`; salida: 200 ms `ease-out`
    - _Requisitos: 11.1, 11.2, 11.4_
  - [ ] 12.2 Crear las funciones `abrirModal()` y `cerrarModal()` en `script.js`
  ...
  - [ ] 12.3 Conectar `abrirModal` al final de `showModal` y `cerrarModal` a los listeners de cierre (botón `.close` y clic en el overlay)
    - _Requisitos: 11.1, 11.3_

- [ ] 13. Checkpoint final — Verificar el Modal rediseñado y las animaciones
  - Asegurarse de que todos los tests pasan, el Modal muestra correctamente los datos y las animaciones de apertura/cierre funcionan sin saltos. Preguntar al usuario si surge alguna duda.
